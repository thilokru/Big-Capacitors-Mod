...
import com.mhfs.capacitors.world.OreGen;
...
public class CommonProxy {
	...
	public void preInit(FMLPreInitializationEvent event, final BigCapacitorsMod mod) {
	...
	private void setupRecipies() {
		...
		// ThermalExpansionHelper.addTransposerFill(80, coilEmptyItem, new
		// ItemStack(Blocks.blockMany, 1, 2), new
		// FluidStack(FluidRegistry.getFluid("cryotheum"), 100), false);
	...
	private void setupNetwork(BigCapacitorsMod mod) {
		...
		mod.network.registerMessage(GuiActionMessage.class, GuiActionMessage.class, 3, Side.SERVER);
	}
	...
	public void init(FMLInitializationEvent event, BigCapacitorsMod mod) {
		...
		mod.fusionReactorMulti = Multiblock.getMultiblock(rl, manager);
		
		rl = new ResourceLocation("big_capacitors:multiblock/crusher.txt");
		mod.crusherMulti = Multiblock.getMultiblock(rl, manager);
